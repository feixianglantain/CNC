...
/*
定义自适应多轴步进平滑级别和截止频率。 最高级别的频率仓始于0Hz，并以其截止频率结束。 
下一个较低级别的频率点从下一个较高的截止频率开始，依此类推。 必须仔细考虑每个级别的截止频率，
以防它过度驱动步进ISR，16位定时器的精度和CPU开销。 级别0（无AMASS，正常运行）的频率槽开始于级别1的截止频率，
并达到CPU所允许的速度（在有限的测试中超过30kHz）。 注意：AMASS截止频率乘以ISR过驱动系数不得超过最大步进频率。 
注意：当前设置设置为将ISR过载驱动至不超过16kHz，以平衡CPU开销和计时器精度。 除非您知道自己在做什么，否则请勿更改这些设置。
...
void st_wake_up()
{
  ...
  HAL_TIM_Base_Start_IT(&htim3); //TIM3->CR1|=0X01;
...
//  TCCR1B = (TCCR1B & ~((1<<CS12) | (1<<CS11))) | (1<<CS10); // Reset clock to no prescaling.
  //bit0=1关闭定时器
  HAL_TIM_Base_Stop_IT(&htim3); //TIM3->CR1&=~0X01;
  busy = false;
...
/* "The Stepper Driver Interrupt" - This timer interrupt is the workhorse of Grbl. Grbl employs
   ...
   此计时器中断是Grbl的主要功能。 Grbl使用古老的Bresenham线算法来管理和精确同步多轴运动。
   与流行的DDA算法不同，Bresenham算法不易受数字舍入误差的影响，仅需要快速的整数计数器，
   这意味着较低的计算开销并最大化了Arduino的功能。但是，对于某些多轴运动，Bresenham算法的缺点是，
   非主导轴可能会遇到不平滑的步进脉冲序列或混叠现象，这可能会导致奇怪的听觉噪声或抖动。这一点特别明显，
   或者可能会在低步进频率（0-5kHz）上引起运动问题，但是在高频下通常不是物理问题，尽管可以听见。
   为了提高布雷森纳姆（Bresenham）多轴性能，Grbl使用了我们所说的自适应多轴步进平滑（AMASS）算法，
   该算法可实现其名称所暗示的功能。在较低的步进频率下，AMASS会在不影响算法固有精度的情况下人为地提高Bresenham分辨率。 
   AMASS根据要执行的步进频率自动调整其分辨率级别，这意味着对于更低的步进频率，步进平滑级别也会增加。
   从算法上讲，通过简单地将每个AMASS级别的Bresenham步数进行位移位即可实现AMASS。例如，对于第1级阶跃平滑，
   我们对Bresenham阶跃事件计数进行位移位，将其有效乘以2，而轴阶跃计数保持不变，然后使步进ISR频率加倍。
   实际上，我们允许非主导的布雷森汉姆轴在中间ISR滴答声中步进，而主导轴是每两个ISR滴答声步进一次，
   而不是传统意义上的每个ISR滴答声步进。在AMASS 2级，我们只需再次移位即可，
   这样非主导的Bresenham轴可以在四个ISR刻度中的任何一个步进，主导轴每四个ISR刻度步进，
   并且将步进ISR频率提高四倍。等等。实际上，这实际上消除了Bresenham算法带来的多轴混叠问题，
   并且不会显着改变Grbl的性能，但实际上，可以在所有配置中更有效地利用未使用的CPU周期。 
   AMASS要求无论AMASS级别如何，始终执行完整的Bresenham步骤，从而保留了Bresenham算法的准确性。
   这意味着对于AMASS 2级，必须完成所有四个中间步骤，以便始终保留基线Bresenham（0级）计数。
   同样，AMASS 3级意味着必须执行所有八个中间步骤。尽管AMASS级别实际上是任意的，基线Bresenham计数可以乘以任何整数值，
   但乘以2的幂只能用于通过位移位整数运算来减轻CPU开销。这种中断是设计简单且愚蠢的。如确定加速度一样，
   所有计算上的繁重工作都在其他地方执行。该中断从步段缓冲区弹出预定义的段（定义为n个步长上的恒定速度），
   然后通过Bresenham算法通过适当地使步进销脉冲来执行它们。步进端口复位中断支持该ISR，它用于在每个脉冲后复位步进端口。
   布雷森汉姆线跟踪器算法通过这两个中断同时控制所有步进器输出。注意：此中断必须尽可能高效，并在下一个ISR滴答之前完成，
   对于Grbl，此滴必须小于33.3usec（@ 30kHz ISR速率）。示波器在ISR中测得的时间通常为5usec，最大为25usec，远低于要求。
   注意：此ISR期望每个段至少执行一个步骤。
*/
...
#endif
{
  if (busy) { return; } // The busy-flag is used to avoid reentering this interrupt busy标志用于避免重新进入此中断

  // Set the direction pins a couple of nanoseconds before we step the steppers 在步进步进机之前，将方向销设置几纳秒
#ifndef STM32
  DIRECTION_PORT = (DIRECTION_PORT & ~DIRECTION_MASK) | (st.dir_outbits & DIRECTION_MASK);
#else
  X_DIRECTION_PORT = (X_DIRECTION_PORT & ~X_DIRECTION_MASK) | (st.dir_outbits & X_DIRECTION_MASK);
  Y_DIRECTION_PORT = (Y_DIRECTION_PORT & ~Y_DIRECTION_MASK) | (st.dir_outbits & Y_DIRECTION_MASK);
  Z_DIRECTION_PORT = (Z_DIRECTION_PORT & ~Z_DIRECTION_MASK) | (st.dir_outbits & Z_DIRECTION_MASK);
  ...
  #else
    X_STEP_PORT = (X_STEP_PORT & ~X_STEP_MASK) | st.step_outbits;
	Y_STEP_PORT = (Y_STEP_PORT & ~Y_STEP_MASK) | st.step_outbits;
	Z_STEP_PORT = (Z_STEP_PORT & ~Z_STEP_MASK) | st.step_outbits;
  #endif
  ...
  sei(); // Re-enable interrupts to allow Stepper Port Reset Interrupt to fire on-time.
         // 重新启用中断以允许步进端口重置中断按时触发
         // NOTE: The remaining code in this ISR will finish before returning to main program.
		 //ISR中的其余代码将在返回主程序之前完成
#else
...
#else
       TIM3->ARR = st.exec_segment->cycles_per_tick;
#endif		
...
void TIMER0_OVF_vect(void)
{
	...
	Z_STEP_PORT = (Z_STEP_PORT & ~Z_STEP_MASK) | (step_port_invert_mask & Z_STEP_MASK);
	
	HAL_TIM_Base_Stop_IT(&htim4); //TIM4->CR1 &= ~0X01;
...
#else
  X_STEP_PORT = (X_STEP_PORT & ~X_STEP_MASK) | step_port_invert_mask;
  Y_STEP_PORT = (Y_STEP_PORT & ~Y_STEP_MASK) | step_port_invert_mask;
  Z_STEP_PORT = (Z_STEP_PORT & ~Z_STEP_MASK) | step_port_invert_mask;
  
  X_DIRECTION_PORT = (X_DIRECTION_PORT & ~X_DIRECTION_MASK) | dir_port_invert_mask;
  Y_DIRECTION_PORT = (Y_DIRECTION_PORT & ~Y_DIRECTION_MASK) | dir_port_invert_mask;
  Z_DIRECTION_PORT = (Z_DIRECTION_PORT & ~Z_DIRECTION_MASK) | dir_port_invert_mask;  
